...
def build_osaka_sample(row: dict) -> dict[str, bytes]:
    if row["prefecture_code"] != "27" or row["comparison_status"] != "comparable":
        raise ValueError("Osaka sample requires comparable canonical prefecture_code 27")
...
'''
    observation_width=round(508 * obs / obs)
    baseline_width=round(508 * base / obs)
...
<desc id="desc">花芽量{obs}個毎平方メートル、過去平均{base}個毎平方メートル、公式比較率{ratio}パーセント。</desc>
<rect width="800" height="420" fill="white"/>
<text x="48" y="55" font-family="sans-serif" font-size="28">{name} スギ雄花花芽量 2025</text>
<text x="48" y="100" font-family="sans-serif" font-size="18">花芽量 {obs:,} 個/m² / 過去平均 {base:,} 個/m² / 公式比較率 {ratio}%</text>
...
<text x="48" y="215" font-family="sans-serif" font-size="16">過去平均</text><rect x="180" y="195" width="{baseline_width}" height="28" fill="#999"/><text x="353" y="216" font-family="sans-serif" font-size="16">{base:,}</text>
...
def main() -> int:
    ...
    osaka=next(r for r in rows if r["prefecture_code"] == "27")
    osaka_dir=SAMPLES / "osaka"
    osaka_dir.mkdir(parents=True, exist_ok=True)
    for name,data in build_osaka_sample(osaka).items():
        (osaka_dir/name).write_bytes(data)

    print(json.dumps({"prefectures":len(rows),"comparable":len(comparable),"output":str(OUT),"sample":str(osaka_dir)},ensure_ascii=False))
    return 0
