...
環境省ホームページでは、特記がないコンテンツに「公共データ利用規約（第1.0版）」が適用されます。規約上、商用利用を含む複製・公衆送信・翻案等が可能で、出典の記載が必要です。編集・加工した場合は、編集・加工したこととその主体も記載する必要があります。第三者が権利を持つ部分や別の利用条件が明示された部分は、その条件を優先します。

このrepositoryでは、環境省資料1の都道府県別表をCSVへ転記・構造化しているため、原資料そのものではなく加工データとして扱います。利用条件の正本は `data/official/moe-source-metadata.json` に保持し、利用時は環境省の現行規約も確認してください。
