...
Cedar Pollen BIは、環境省が公開するスギ雄花花芽調査をもとに、都道府県別の花芽量を過去10年平均と比較して表示する公開データBIです。調査年、基準期間、単位、比率、BIといった技術的な読み方は、この観測対象を理解した後で扱います。

READMEの入口は [`KAFKA2306/articles#34`](https://github.com/KAFKA2306/articles/issues/34) の「広い問題 → 具体例 → 技術」の編集原則を維持します。花芽量、飛散実測、飛散予測、症状リスクを混同せず、出典・調査年・基準期間・単位が欠ける値を推測で補完しません。
